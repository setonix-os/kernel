...
written constitution that most projects do not, and because CLAUDE.md §1 and §5.4 are the two rules
...
toil (tooling, tests, docs, build). **If it is neither, that is the finding.** CLAUDE.md §1 is
...
CLAUDE.md §3: the kernel provides mechanism and never policy. Look for policy that has crept below
...
CLAUDE.md §4 and §11.2. For each subsystem touched:
...
CLAUDE.md §5.2 and §11.4: nothing merges un-understood. Judge whether the change ships an
...
CLAUDE.md §11.5: new files carry a GPLv3 SPDX header; vendored MIT code keeps its notices verbatim.

## 7. Scope

CLAUDE.md §6. Flag anything that quietly serves a non-goal — desktop polish, bug-for-bug POSIX
...
resolving it in either direction (CLAUDE.md §11.1).
